...
createUser() {
  ...
  sudo adduser \
     --system \
     --shell /bin/bash \
     --gecos 'User for managing cmoc stack' \
     --group \
     --disabled-password \
     --home /home/${user} \
...
chkStatus "sudo su - ${user} \"cat >foo <<EOF
...
chkStatus "sudo su - ${user} \"gpg --batch --generate-key foo\"" "generate gpg key"

sudo su - ${user} "cat >  ~/.netrc <<EOF
machine github.com
login ${GITHUB_USER}
...
chkStatus "sudo su - ${user} \"gpg -e -r ${user} ~/.netrc\"" "encrypt temporary ~/.netrc with your key"
chkStatus "sudo su - ${user} \"rm -f ~/.netrc\"" "delete temp ~/.netrc"

sudo su - ${user} "curl --silent -o ~/.local/bin/git-credential-netrc https://raw.githubusercontent.com/git/git/master/contrib/credential/netrc/git-credential-netrc"

chkStatus "sudo su - ${user} \"git config --local user.name ${GITHUB_USER}\"" "set git user"
chkStatus "sudo su - ${user} \"git config --local user.email ${GITHUB_EMAIL}\"" "set git email"

sudo su - ${user} "cat >~/.profile<<\EOF
...
chkStatus "sudo su - ${user} \"ln -sf ${copyDir} ~/cloud\"" "symlink ~/cloud"

}

####### Installation Start

chkStatus "mkdir -p $copyDir;ln -s $copyDir $envDir" "create directories"

######## Prepare System
chkStatus "sudo yum -y update" "${blu} Updating OS ${end}"
chkStatus "sudo yum -y install git jq" "installing tools"

chkStatus "sudo groupadd cmocdev" "create cmocdev group"
